...
const Items = ({ifExists, id, title, text, btn, img, price, rating, color, shadow }) => {
          ...
          >
              ...
              <div className="flex items-center justify-between w-28 my-2">
                ...
                <div className="flex items-center gap-1">
                  <StarIcon className="icon-style w-5 h-5 md:w-4 md:h-4" /><h1 className="md:text-sm font-normal text-slate-100">{rating}</h1>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <button type='button' onClick={() => {onAddToCart(); onCartToggle();}} className="bg-white/90 blur-effect-theme button-theme px-2 py-1 shadow shadow-sky-200 text-sm text-black">{btn}</button>
              </div>
          </div>
